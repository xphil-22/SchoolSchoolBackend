...
class Week:
    def __init__(self, html:str) -> None:
        self._days:list[Day] = [] #List with days
        self._day_acronyms:list[str] = ['mon','tue','wed','thu','fri'] #day acronyms
    ...
    def getWeekMeals(self) -> list[dict[str]]: #call getTodaysMeals on every day and return each value saved in a list
        return [day.getTodaysMeals() for day in self._days]
        
    def getTodaysMeal(self) -> list[dict[str]]: #Returns todays or next mondays Meal (If Weekday is Saturday or Sunday)
